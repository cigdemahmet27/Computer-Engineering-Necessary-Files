...
{
    TreeNode* temp = *root;
    *root = temp->children[move];

    for(int i = 0; i < temp->num_children; i++)
    {
        if(temp->children[i] != *root)
        {
            free_tree(temp->children[i]);
        }
    }
    free(temp->children);
    free_game_state(temp->game_state);
    free(temp);

    expand_tree(*root);

    if (node_count(*root) < pow((*root)->game_state->width, initial_tree_depth - 2))
    {
        expand_tree(*root);  
    }
    ...
    while (get_game_status(root->game_state) == IN_PROGRESS)
    {
        ...
        if (root->game_state->next_turn) // Player 2
        {
            if (play_against_human)
                ...
                //move = best_move(root);//get_random_move(root->game_state);
        }
        else // Player 1
        {
            // Lets play the first 5 moves randomly since the tree is very symmetric for the first few moves
