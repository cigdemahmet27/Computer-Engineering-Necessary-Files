...
int best_move(TreeNode* root)
{
    ...
    if(root->game_state->next_turn)
    {
        ...
        {
            int rand_index = 0;
            int max_score = 1001;//get_max(root->children[rand_index]);

            for(int i = 0; i < root->num_children; i++)
            {
                int temp = get_max(root->children[i]); 
                if(temp < max_score)
                {
                    max_score = temp;
                    rand_index = i;
        ...
        {
            int rand_index = 0;
            int min_score = -1001;//get_min(root->children[rand_index]);

            for(int i = 0; i < root->num_children; i++)
            {
                int temp = get_min(root->children[i]);
...
int get_max(TreeNode* node)
{
    ...
    {
        int GameScore = -1001; //get_min(node->children[rand() % node->num_children]);

        for(int i = 0; i < node->num_children; i++)
        {
            int k = get_min(node->children[i]);
...
int get_min(TreeNode* node)
{
    if(node->num_children > 0)
    {
        int GameScore = 1001; //get_max(node->children[rand() % node->num_children]);

        for(int i = 0; i < node->num_children; i++)
        {
            int k = get_max(node->children[i]);
